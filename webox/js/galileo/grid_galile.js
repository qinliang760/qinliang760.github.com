(function($){
	...
	mediav.Grid = function(box, options){
		...
		//debugger;
		var table_html, cols_attr = [],	htmls = [], w,lastcss=""/*, firstcss = "", firstIndex = 1*/,sort_rel="";	
		...
		var last_index = this.settings.cm.length - 1;
		$.each(this.settings.cm, function(i, v){	
			if ( last_index== i)
			{
				lastcss = ' class="noBorderRight"';
			}		  
			if(v.sortable){
			...
			if(v.editable){
				editable_class=' class="edit"';	
			}else{
				editable_class="";	
			}
			Cls.tpl_row += ('<td align="{#align}" {#editable_class}{#lastcss}>{#{#name}}</td>'.supplant({
				"name": v.name,
				"editable_class": editable_class,
				"align": v.align,
				"lastcss": lastcss
			}));
			htmls.push('<th align="{#align}" nowrap="nowrap"{#sort_class} rel="{#name}">{#title}</th>'.supplant({
		...
		if (this.show_pagebar)
		{
			...
			htmls.push('<tr><td width="100">&nbsp;</td><td>');
			htmls.push('<div class="pageArea clearFix"><span class="pagebarTotal"></span><p class="pagebarSize"><span>每页显示</span><select><option>10</option><option>25</option><option>50</option></select></p><a class="prevBtn prevBtnDis" href="javascript:;" rel="prev">上一页</a> <span class="pagebarStat"></span> <a class="nextBtn nextBtnDis" href="javascript:;" rel="next">下一页</a></div>');
			htmls.push('</td></tr>');
			htmls.push('</table>');
		}
...
/*		$(".btnStyle").hover(function(){
		...
		$("table.pagebar a", this.j_box).live("click", function(){
			if($(this).hasClass("prevBtnDis") || $(this).hasClass("nextBtnDis")){
				...
				var size=$("table.pagebar .pageArea .pagebarSize select", this.j_box).val();
				//console.log(page);
				Cls.settings["handlePage"](page,size);					
			}													

			
		});
		//add size
		$("table.pagebar .pageArea .pagebarSize select", this.j_box).change(function(){
				var size = $(this).val();
				//console.log(size);
				Cls.settings["handleSize"](size);
...
/*		if(this.settings.editable){
			var cmLength=Cls.settings.cm.length;
			for(i=0;i<cmLength-1;i++){
				if(Cls.settings.cm[i].editable){
					$(".mvgrid table.grid tr").each(function(){
						$(this).find("td").eq(i).addClass("edit");										 
					})	
				}
			}
		}*/
		...
		$(".mvgrid table.grid th.sort", this.j_box).live("click", function(){
			var thIndex=$(this).parent().find("th").index(this);
			...
			if(Cls.show_pagebar){
				if(Cls.settings.cm[cmIndex].sortable){
					...
					if($(this).parents(".mvgrid").find("table.pagebar a[rel=next]").hasClass("nextBtnDis")){
						var page =$("table.pagebar a[rel=next]").attr("pageindex");	
					}else{
						var page =$("table.pagebar a[rel=next]").attr("pageindex")-1;						
					}
					var size=mediav.Grid.prototype.getSize();
					Cls.settings["handlePage"](page,size);
	/*				$(".mvgrid table.grid tr").each(function(){
		...
		"setData" : function(json){
			...
			$.each(json.result, function(i, v){		
				...
				for (var v2 in v)
				{
					...
					//添加表格行底色
					if(v.trclass=="trParentSign"){
						v.trSign='class="trParentSign"';	
					}else if(v.trclass=="trChildSign"){
						v.trSign='class="trChildSign"';
					}else{
						v.trSign='class=""';		
					}
...
//			$("table.grid tr:first", self.j_box).after(htmls.join(""));
			...
			//$("table.grid tr:odd", this.j_box).addClass("odd");
			//细分不可编辑控制
			$("table.grid tbody td.edit", self.j_box).each(function(){
				if($(this).parent().attr("id")==$(this).parent().prev().attr("id")){
					$(this).removeClass("edit");	
				}														
			})			
			//表格可编辑样式
			$("table.grid tbody td.edit", self.j_box).hover(function(){
																	 	
				$(this).css("background-position","right -220px");														 
			},function(){
				$(this).css("background-position","-999px -999px");	
			})
			
			//表格缩进
			$(".trChildSign").each(function(){
				$(this).find("td:first").css("text-indent","2em");							
			})
		...
		"getSize":function(){
			var sizeNum=$("table.pagebar .pageArea .pagebarSize select", this.j_box).val();
			return sizeNum;
		},
		/**
		...
		"pagination" : function(data)
		{
			...
			$("span.pagebarTotal", j_pagebar).html("共有"+data.total+"行");
			//alert(globalPageSize);
			$(".pagebarSize select option", j_pagebar).each(function(){//debugger;
				if($(this).text()==data.size){
					$(this).attr("selected","selected").siblings().removeAttr("selected");
				}											  
			})
			//$(".pagebarSize select option:eq(2)").attr("selected","selected");
			//debugger;
