    ...
    $.extend({
        ...
        jqotec: function(template, t) {
            var cache, elem, tmpl,
                ...
                ie = $Browser.ie && (parseInt($Browser.ie, 10) < 9);
            t = t || tag;
...
(function() {
	 ...
	 * 消息队列，用于监听、触发全局自定义事件
	 */
	function MQ(_key, _immediately) {
		var that = this;
		/**
		 * key，
		 * @param {boolean}  _immediately 是否立即执行，默认false
		 */
		that.init = function(_key, _immediately) {
			that._key = _key || "global";
			this._immediately = _immediately;
		};

		/**
		 * 添加一个监听，
		 * @param   {function}    _func 接收notify时的_settings数据
		 * @param {string} _namespace 绑定的命名空间的名称
		 */
		that.bind = function(_func, _namespace) {
			var key = this._getKey(_namespace);
			$(this).bind(key, _func);
		};
		/**
		 * 获取Key
		 */
		that._getKey = function(_namespace) {
			var key = this._key;
			if (typeof _namespace == 'object') {
				key = key + "." + _namespace.getCreateTime();
			} else if (_namespace) {
				key = key + "." + _namespace;
			}
			return key;
		...
		/**
		 * 解绑已有的监听
		 * @method  .unbind(key,[function]);
		 * @author  hite
		 * @version 1.0
		 * @date    2012-5-30
		 * @param   {string}    _namespace  解绑的命名空间的名称
		 * @param   {function}    _func 如果相同事件有不同的监听时，支持解绑特定函数
		 * @return  {void}          无返回值
		 */
		that.unbind = function(_namespace, _func) {
			var key = this._getKey(_namespace);
			if (_func) {
				$(this).unbind(key, _func)
			} else {
				$(this).unbind(key);
			}
		};
		...
		/**
		 * 调用所有监听方法，
		 * @param   {object}    _settings 传递给所有监听方法
		 * @param {string}  _namespace
		 */
		that.notify = function(_settings, _namespace) {
			var that = this;
			if (_namespace) {
				var eventType = that._key + "." + _namespace;
			} else {
				var eventType = that._key;
			}

			var key = '$MQ.' + that._key;
			if (that._immediately) {
				try {
					$(that).trigger(eventType, _settings);
				} catch (ex) {
					throw ex;
				}
			} else {
		...
		that.getCount = function() {
			...
			for (var key in that) {
				if (key.indexOf('jQuery') === 0) {
					return that[key].events[that._key].length;
				}
			}
			return 0;
		}

		that.init(_key, _immediately);
	};
	...
	$MQ = {
		// RESIZE
		RESIZE: new MQ("RESIZE"),
		// 时间变化了
		TIMECHANGE : new MQ("TIMECHANGE"),
		// 需要登录
		REQUIRELOGIN : new MQ("REQUIRELOGIN"),
		// 触发lazyload图片加载
		LAZYLOAD : new MQ("LAZYLOAD"),
        // 滚动事件
        SCROLL : new MQ("SCROLL")
	};
 ...
 * 浏览器版本，this.ie/firefox/chrome/opera/safari,如果浏览器是IE的，this.ie的值是浏览器的版本号，!this.ie表示非IE浏览器
 */
 ...
 */
define('widget/helper',function(require, exports, module) {
    var pIndex = 64;
    // dropmenu
    var pLastPopup = [];
    // hovermenu
     ...
     */
    function getTotalScrollTop (ele){
        var scrollTop = 0;
        ...
        if(parEle && parEle.get(0)) {
            // 有父节点，网上追溯
            return scrollTop + getTotalScrollTop(parEle);
        } else {
            return scrollTop;
        }
    }
    ...
    var tool = {
        /**
         * 外部停靠，可以指定 停靠的位置，如top,left,middle,center,bottom,right等
         * <br/>默认为middle,center.
         * @example .dockIn(ship, dock, { axis: "middle,center", positionFix : {x : 0, y:0}});
         * @author  hite
         * @version 1.0
         * @date    2012-3-27
         * @param   {object|jqueryObject}  ship  待停靠的元素。
         * @param   {object|jqueryObject}  dock 停靠的父元素
         ...
         * @return  {void}            无返回值
         */
        dockV : function(ship, dock, settings) {
            var that = this;
            
            var x_axis = '';
            var y_axis = '';
            settings = $.extend({axis : 'left,bottom'}, settings);
            ...
            // 修正滚动条
            var topScroll = getTotalScrollTop(ship);
            finalTop += topScroll;
            // 左右调节
         ...
         * @return  {void}            无返回值
         */
        dockIn:function(ship, dock, settings){
            var that = this;
            ...
            var dock = dock || $(window);
            // dock
            var dockWidth = dock.outerWidth();
            var dockHeight = dock.outerHeight();

            // 滚动条修正
            var scrollTop = dock.scrollTop();
            ...
            finalLeft += positionFix.x;
            finalTop += positionFix.y;
            if(settings.dockFunc === 'css'){
                ship.css({left:finalLeft, top : finalTop});
            }else{
                ship.offset({left:finalLeft, top : finalTop});
            }
        },
        addPop:function(ppnl){
            var that = this;
         ...
         */
        clearPop:function(_site){
            var target= $(_site);
            for (var i = pLastPopup.length - 1; i >= 0; i--) {
                var popupEle = pLastPopup[i];
                if(popupEle.hasClass('f-ani-ppnlmenu')) {
                    if(target.length == 0 || (target.length>0 && !$.contains(popupEle.get(0), target.get(0)))){
                        // 回调
                        try {
                            popupEle.removeClass('f-ani-ppnlmenu');
                            if(popupEle.hasClass('js-dropmenu-ppnl')) {
                                // dropmenu弹出的 直接隐藏
                                popupEle.addClass('f-hide');
                            } else {
                                setTimeout(function() {
                                    popupEle.addClass('f-hide');
                                }, 200);
...
/**
    demo
    var loginDialog = $($('#loginTemplate').html());
    var dialog = loginDialog.showDialog({dialogClsName:"f-grouplabelwid-4", onclose : function(){}});
    dialog.closeDialog();
*/

define('widget/dialog',function(require, exports, module) {
    var Log = require('tool/log');
    return function(jquery){
         (function($) {
            var pBaseTmpl =
            '<div class="m-overlay m-overlay-ani <%=this.dialogClsName||""%>">\
                <div class="w-mask w-mask-ani js-mask"></div>\
                <div class="overlay-container f-scroll-y overlay-container-ani f-tlbr js-overlay-container">\
                            ...
                            <div class="m-hrz">\
                                ...
                                <div class="m-wrap">\
                                    <div class="w-txticon f-ribs">\
                                        <span class="txt f-size-middle">\
                                            <%=this.title%>\
                                        </span>\
                                    </div>\
                                </div>\
                                <%}%>\
                                <div class="m-wrap m-wrap-right">\
                                    <a href="javascript:;" hidefocus="true" class="w-linkicon js-close-pop" title="关闭">\
                                        <span class="w-icon icon-close-pop"></span>\
                                    </a>\
                                </div>\
                            </div>\
                        </div>\
            ...
            var pZindex = 1130,
                pZIndexStep = 20,
                pDialogContainer = null;

            var helper = require('widget/helper');
            $.fn.extend({
                showDialog : function(options) {
                    var dom = this;
                    var container, body, mask, overlay, content;
                    prepare();
                    container = $($.jqote(pBaseTmpl, options));
                    body = container.find('.js-w-dialog-body');
                    mask = container.find(">.js-mask");
                    overlay = container.find(">.js-overlay-container");
                    content = body.find('>.js-w-dialog-content');

                    content.append(dom);

                    pDialogContainer.append(container);
                    // ie 6 设置高度
                    if($Browser.ie && parseInt($Browser.ie, 10) === 6) {
                        pDialogContainer.children().height($(document.body).height());
                    }
                    // 定位
                    helper.dockIn(body, $(window), {
                    ...
                    pZindex += pZIndexStep;
                    container.css("z-index", pZindex);

                    container.removeClass('f-ani-overlay');
                    mask.addClass('f-ani-mask');
                    overlay.addClass('f-ani-bouncein');

                    body.find('>.js-w-dialog-head .js-close-pop').bind('click', function(){
                        container.closeDialog(options);
                    });
                    // 绑定input
                    $('[data-plugin="input"]', container).each(function(i, d) {
                        $(d).startInput();
                    });
                    return container;
                },
                closeDialog : function(options) {
                    var container = this;
                    container.addClass('f-ani-overlay');
                    // 动画结束的时候，再移除样式
                    setTimeout(function() {
                        mask = container.find(">.js-mask");
                        overlay = container.find(">.js-overlay-container");
...
/**
    demo
    var form = $('form');
    var dialog = form.formValidate();
*/

define('widget/form',function(require, exports, module) {
    var Validate = require('tool/validate');
    return function(jquery){
         (function($) {
            function showError(ele, msg) {
                var errorWrap = ele.parents('.js-error-wrap');
                errorWrap.addClass('m-wrap-err f-ani-flash');
                errorWrap.find('.js-errorTip').html(msg);
                return new Error(msg);
            }

            function hideError(ele) {
                var errorWrap = ele.parents('.js-error-wrap');
                errorWrap.removeClass('m-wrap-err f-ani-flash');
                errorWrap.find('.js-errorTip').html('');
            }
            $.fn.extend({
                validateInput : function() {
                    ...
                    if(el && el.tagName === 'INPUT' || el.tagName === 'TEXTAREA') {
                        ...
                        if(ele.attr('required')) {
                            if(val === "") {
                                // 空字符串
                                return showError(ele, '不能为空');
                            }
                        }
                        var minLength = ele.attr('minLength');
                        if(minLength) {
                            if(val.length < parseInt(minLength, 10)) {
                                // 长度不够
                                return showError(ele, '不能少于' + minLength + '字符');
                            }
                        }
                        var maxLength = ele.attr('maxLength');
                        if(maxLength) {
                            if(val.length > parseInt(maxLength, 10)) {
                                // 长度不够
                                return showError(ele, '不能多于' + maxLength + '字符');
                            }
                        }
                        ...
                        if(type === 'number') {
                            ...
                            if(value + '' !== val) {
                                return showError(ele, '请输入数字');
                                // 不是数字
                            }
                        } else if(type === 'email') {
                            if(!Validate.isEmail(val)) {
                                return showError(ele, '请输入正确的email格式');
                                // 不是Email
                            }
                        } else if(type === 'url') {
                            // if(!urlReg.test(val)) {
                            ...
                            value = Date.parse(val);
                            if(typeof value !== 'number') {
                                // 不是时间
                                return showError(ele, '请输入正确的时间格式如 2015-10-33 10:10');
                            }
                        }
                        ...
                        // 检查格式
                        var checkType = ele.attr('data-checktype');
                        if(checkType === 'mobile') {
                            if(!Validate.isCellPhone(val)) {
                                return showError(ele, '请输入正确的手机号码格式');
                                // 不是Email
                            }
                ...
                formValidate: function() { 
                    ...
                    if(form && form.tagName === 'FORM') {
                        if(!this.data('__validateInit')) {
                            this.data('__validateInit', true);
                            this.delegate('INPUT,TEXTAREA,SELECT', 'input propertychange', function(evt) {
                                hideError($(evt.currentTarget));
                            });
                        }
...
/**
    demo
    var form = $('select');
    var dialog = form.render(datalist, selectedValue);
*/

define('widget/select',function(require, exports, module) {
    var tool = require('tool/tool');
    return function(jquery){
        (function($) {
            $.fn.extend({
                render: function(datalist, selectedValue) { 
                    var select = this.get(0);
                    if(select && select.tagName === 'SELECT') {
                        if(this.data('__renderInit')) {
                            return;
                        }
                        this.data('__renderInit', true);
                        var html = '';
                        for (var i = 0; i < datalist.length; i++) {
                            var option = datalist[i];
                            html += '<option value ="' + tool.escapeHTML(option.value) + '"';
                            if(option.value === selectedValue) {
                                html += ' selected="selected"';
                            }
                            html += '>';
                            html += tool.escapeHTML(option.name);
                            html += '</option>';
                        };
                        this.html(html);
                    }
...
define('widget/countdown',function(require, exports, module) {
    var Tool = require('tool/tool');
	var Time = require('tool/time');
	...
	function defaultFormat(status, dom, day, hour, minute, second) {
        var tip = '';

        if(status === -1) {
            // 未开始
            if(day > 0) {
                var startTime = parseInt(dom.attr('data-since'), 10);
                tip = Time.formatDate(startTime, 'M月d日 HH:mm 开始');
            } else {
                tip = '距开始还有: ';
                if(hour > 0) {
                    tip += hour + '小时';    
                ...
                if(hour > 0 || minute > 0) {
                    tip += Tool.paddingLeft(minute, 2, '0') + '分';
                }
                if(hour > 0 || minute > 0 || second > 0) {
                    tip += Tool.paddingLeft(second, 2, '0') + '秒'
                }
            }
        } else if(status === 0) {
            tip = '还剩: ';
        ...
        (function($) {
            $.fn.extend({
                countdown: function(callback) { 
                	var dom = this;
                	var startTime = parseInt(dom.attr('data-since'), 10);
                    var endTime = parseInt(dom.attr('data-until'), 10);
                    if(isNaN(startTime)) {
                    	startTime = 1;
                    }
                    ...
                    if(startTime > 0 && endTime > 0) {
                    	var mqKey = "mq_" +  Math.random();
                    	$MQ.TIMECHANGE.bind(function() {
                    		var now = Time.getServerTime();
                    		var title = "";
                            var res;
                            var html;
                    		if(now < startTime) {
                                res = getCountdown(startTime - now);
                                callback(-1, dom, res.day, res.hour, res.minute, res.second);
                    		} else if(now > endTime) {
                    			// 已经结束
	                			$MQ.TIMECHANGE.unbind(mqKey);
                                callback(1, dom);
                    		} else {
                    			// 正在进行，倒计时
                    			res = getCountdown(endTime - now);
                                callback(0, dom, res.day, res.hour, res.minute, res.second);
                    		}
                    	}, mqKey);
                    }
                }
...
define('widget/dropmenu',function(require, exports, module) {

    var helper = require('widget/helper');
    return function(jquery){
        (function($) {
            $.fn.extend({
                dropMenu: function(ppnl, dockSettings, settings) {
                    var dom = this;

                    if(!dom.data('__bindDropMenu')) {
                        dom.data('__bindDropMenu', true);
                        
                        dom.bind('click', function() {
                            ppnl.removeClass('f-hide');

                            ppnl.addClass('js-dropmenu-ppnl');
                            // 显示
                            setTimeout(function() {
                                ppnl.addClass('f-ani-ppnlmenu');
                            }, 30);
                            // 设置settings
                            // 如果有箭头
                            if(ppnl.find('.w-arrow-up').length > 0) {
                                var defSettings = {
                                    axis : 'right,bottom',
                                    positionFix :{x:10,y:15}
                                };
                            } else {
                                var defSettings = {};
                            }
                            dockSettings = $.extend(defSettings, dockSettings);
                            helper.dockV(ppnl, dom, dockSettings);
                            helper.addPop(ppnl);
                        });
                    }
                },
                ...
                dropOut: function(ppnl) {
                    ...
                    if(!dom.data('__bindDropMenu')) {
                        ...
                        dom.bind('click', function() {
                            ppnl.addClass('js-dropmenu-ppnl');
                            // 如果已经显示的，则隐藏
                            if(!ppnl.hasClass('f-hide')) {
                                ppnl.addClass('f-hide');
                            } else {
                                ppnl.removeClass('f-hide');
                                // 显示
                                setTimeout(function() {
...
define('widget/hovermenu',function(require, exports, module) {

    var helper = require('widget/helper');

    return function(jquery){
        (function($) {
            function isVisible(ppnl) {
                if(!ppnl.hasClass('f-hide') && ppnl.hasClass('f-ani-ppnlmenu')) {
                    return true;
                }
                return false;
            }
            function setHoverred(ppnl, hoverred) {
                ppnl.data('data-hoverred', hoverred);
            }
            function isHoverred(ppnl) {
                return !!ppnl.data('data-hoverred');
            }
            $.fn.extend({
                isHoverred : function() {
                    var ppnl = this;
                    return isHoverred(ppnl);
                },
                hoverMenu: function(ppnl, dockSettings, settings) {
                    var dom = this;
                    ...
                    if(!dom.data('__bindDropMenu')) {
                        ...
                        function hoverOut(name) {
                            ...
                            ppnl.removeClass('f-ani-ppnlmenu');
                            // 显示
                        ...
                        dom.mouseenter(function() {
                            setHoverred(ppnl, true);
                            hoverInTimeout = setTimeout(function() {
                                hoverIn('trigger');
                            }, 100);
                        });
                        dom.mouseleave(function() {
                            setHoverred(ppnl, false);
                            hoverOutTimeout = setTimeout(function() {
                                hoverOut('trigger');
                            }, 200);
                        });
                        ppnl.mouseenter(function() {
                            setHoverred(ppnl, true);
                            hoverInTimeout = setTimeout(function() {
                                hoverIn('ppnl');
                            }, 100);
                        });
                        ppnl.mouseleave(function() {
                            setHoverred(ppnl, false);
                            hoverOutTimeout = setTimeout(function() {
                                hoverOut('ppnl');
                            }, 200);
...
define('widget/socialshare', function(require, exports, module) {
    var Log = require('tool/log');
    return function(jquery) {
        /**
         * 分享
         * Example:  $(".share").socialshare();
         */
        (function($) {
            /**
             * 新浪微博
             */
            function shareTsina(url, title, description, pic) {
                var p = {
//                    url: url,
                    title: description,
                    appkey: 2526354894,
             ...
             * 腾讯微博
             */
            function shareTqq(url, title, description, pic) {
                var p = {
                    title: description,
                    url: url,
                    appkey: "801106524",
                    pic: pic, //（例如：var _pic='图片url1|图片url2|图片url3....）
                    site: "http://you.163.com" //你的网站地址
                };
                var s = [];
                for (var i in p) {
                    s.push(i + '=' + encodeURIComponent(p[i] || ''));
             ...
             * 腾讯QQ空间
             */
            function shareQQzone(url, title, description, pic) {
                var p = {
                    url: url,
                    desc: description,
                    /*默认分享理由(可选)*/
             ...
             * 豆瓣
             */
            function shareDouban(url, title, description, pic) {
                var p = {
                    url: url,
                    comment: description,
                    /*分享内容(可选)*/
                    title: description,
                    /*分享标题(可选)*/
                    image: pic /*分享图片的路径(可选)*/
                };
                var s = [];
                for (var i in p) {
                    s.push(i + '=' + encodeURIComponent(p[i] || ''));
                }
                return "http://www.douban.com/recommend/?" + s.join('&');
            };

            /**
             * 人人
             */
            function shareRenren(url, title, description, pic) {
                // 人人非注册app只能分享link
                var p = {
                    url: url,
                    pic: pic,
                ...
                return "http://share.renren.com/share/buttonshare/post/1004?" + s.join('&');
            };

            /**
             * 网易微博
             */
            function shareT163(url, title, description, pic) {
                var p = {
                    source: "",
             ...
             * 开心网
             */
            function shareKaixin(url, title, description, pic) {
                var p = {
                    rtitle: title,
                    rurl: url,
                ...
                return "http://www.kaixin001.com/repaste/bshare.php?" + s.join("&");
            };

            /**
             * 分享到微信，二维码方式
             */
            function shareWeixin(url, title, description, pic) {
                var html = '<div class="m-ppnl f-hide m-popwin-shareweixin">';
                    html += '<div class="ppnl-ct js-ppnl-ct">';
                    html += '<div class="js-ppnl-content">';
                    html += '<img src="http://s.jiathis.com/qrcode.php?url=' + encodeURIComponent(url) + '" alt="二维码加载失败...">';
                    html += '</div>';
                    html += '</div>';
                    html += '</div>';
                return html;
             ...
             * 易信分享
             */
            function shareYixin(url, title, description, pic) {
                var p = {
                    appkey: "yxedd5d77076f74e9c88069c5e78acb0aa",
                    type: "webpage",
                    url: url,
                    pic: pic,
                    desc: description,
                ...
                return "http://open.yixin.im/share?" + s.join("&");
            };
            ...
            $.fn.socialshare = function() {
                var dom = this;
                var supported = {
                    shareKaixin: shareKaixin,
                    shareT163: shareT163,
                    shareRenren: shareRenren,
                    shareDouban: shareDouban,
                    shareTqq: shareTqq,
                    shareQQzone: shareQQzone,
                    shareTsina: shareTsina,
                    shareWeixin: shareWeixin,
                    shareYixin: shareYixin
                };
//                var url = '';
                var url = dom.find('[name="url"]').val();
                var description = dom.find('[name="description"]').val();
                var pic = dom.find('[name="pic"]').val();
                var sharetitle = dom.find('[name="sharetitle"]').val();

                var shareWxs = dom.find('.js-share[data-share="shareWeixin"]');
                for (var i = shareWxs.length - 1; i >= 0; i--) {
                    var icn = $(shareWxs[i]);
                    var html = supported['shareWeixin'](url, sharetitle, description, pic);
                    var ppnl = $(html);
                    dom.append(ppnl);
                    icn.hoverMenu(ppnl, {
                        axis : icn.attr('data-axis'),
                        positionFix :{x:-36,y:0}
                    });
                };

                dom.delegate('.js-share', 'click', function() {
                    var funName = $(this).attr('data-share');
                    var logName = $(this).attr('data-logname');
                    Log.log({
                        key: logName
                    });
                    if (funName === 'shareWeixin') {
                        // 微信分享，弹出一个浮层来

                    } else {
                        var linkUrl = supported[funName](url, sharetitle, description, pic);
                        window.open(linkUrl);
                    }
...
/**
    pager
    var btn = $('#ajaxpager');
    btn.ajaxPager({pagination : pagination, onclick(page, defer)})
*/

define('widget/pager', function(require, exports, module) {

    var helper = require('widget/helper');
    return function(jquery) {
        (function($) {
            function getBtn(text, page) {
                return '<div class="m-wrap">\
                    <a href="javascript:;" hidefocus="true" class="w-linkicon" data-page="' + (page) + '">\
                        <span class="txt ">' + text + '</span>\
                    </a>\
                </div>';
            }

            function renderBtns(dom, pagination) {
                dom.empty();
                ...
                // 分页信息 page size totalPage total
                if (currentPage > 0) {
                    var prevBtn = $('<div class="m-wrap">\
                        <a href="javascript:;" hidefocus="true" class="w-linkicon" data-page="' + (currentPage - 1) + '">\
                        <span class="w-icon icon-pagel"></span>\
                        <span class="txt"></span>\
                        </a>\
                    </div>');
                    dom.append(prevBtn);
                ...
                for (var i = 0; i < currentPage; i++) {
                    if (i < 2 || i > currentPage - 3) {
                        dom.append(getBtn(i + 1, i));
                    } else if (i == 2) {
                        dom.append(getBtn('...', Math.ceil((0 + currentPage) / 2)));
                    } else {
                        continue;
                    }
                };
                ...
                // 选中按钮
                var currentBtn = $('<div class="m-wrap">\
                    <a href="javascript:;" hidefocus="true" class="w-linkicon w-linkicon-selected" data-page="' + (currentPage) + '">\
                    <span class="txt">' + (currentPage + 1) + '</span>\
                    </a>\
                </div>');
                dom.append(currentBtn);

                // 后面
                for (var i = currentPage + 1; i < pagination.totalPage; i++) {
                    if (i < currentPage + 3 || i > pagination.totalPage - 3) {
                        dom.append(getBtn(i + 1, i));
                    } else if (i == currentPage + 3) {
                        dom.append(getBtn('...', Math.ceil((currentPage + pagination.totalPage) / 2)));
                    } else {
                        continue;
                    }
                };
            ...
            $.fn.extend({
                ajaxPager: function(settings) {
                    var dom = this;
                    // 分页信息 page size totalPage total
                    var pagination = settings.pagination;
                    // 点击事件
                    var onclick = settings.onclick || function(page) {
                        alert('goPage : ' + page);
                    };
                    dom.undelegate('a', 'click');
                    dom.delegate('a', 'click', function(evt) {
                        var page = parseInt($(evt.currentTarget).attr('data-page'), 10);

                        var defer = $.Deferred();
                        defer.done(function(res) {
                            renderBtns(dom, res.data.pagination);
                        });
                        onclick(page, defer);
                    });

                    renderBtns(dom, pagination);
                }
            });
        })(jquery);
    };
...
define('widget/regsuggest',function(require, exports, module) {

    var helper = require('widget/helper');
    return function(jquery){
        (function($) {
            $.fn.extend({
                regSuggest: function(ppnl, dockSettings, settings) {
                    var dom = this;

                    if(!dom.data('__bindRegsuggest')) {
                        dom.data('__bindRegsuggest', true);
                        var input = dom.find('input[type="text"]');
                        ppnl.delegate('.hl-item', 'mouseover', function(evt) {
                            var item = $(evt.currentTarget);
                            item.siblings('.hl-item-hover').removeClass('hl-item-hover');
                            item.addClass('hl-item-hover');
                        });
                        ppnl.delegate('.hl-item', 'mouseout', function(evt) {
                            var item = $(evt.currentTarget);
                            item.removeClass('hl-item-hover');
                        });

                        function selectItem(item) {
                            input.val(item.find('a .txt').text());

                            ppnl.removeClass('js-dropmenu-ppnl');
                            ppnl.addClass('f-hide');
                        };
                        // 选中
                        ppnl.delegate('.hl-item', 'mousedown', function(evt) {
                            var item = $(evt.currentTarget);
                            selectItem(item);
                        });
                        ...
                        input.bind('keydown', function(evt) {
                            if(evt.keyCode == 13) {
                                // 回车
                                var selected = ppnl.find('.hl-item-selected:first');
                                ...
                                if(prev.length > 0) {
                                    prev.addClass('hl-item-selected');
                                    selected.removeClass('hl-item-selected');
                                }
                                return false;
                                ...
                                if(next.length > 0) {
                                    next.addClass('hl-item-selected');
                                    selected.removeClass('hl-item-selected');
                                }
                                return false;
                            } 
                        ...
                        input.bind('keyup', function(evt) {
                            ...
                            if(val.length > 0) {
                                var selected = false;
                                ppnl.find('.hl-item').each(function(index, item){
                                    var itemBlock = $(item);
                                    var userName = val.split('@')[0];
                                    var suffix = $(itemBlock.find('a')).attr('data-suffix');
                                    itemBlock.find('a .txt').text(userName + suffix);

                                    itemBlock.removeClass('hl-item-hover');

                                    if((userName + suffix).indexOf(val) === 0) {
                                        itemBlock.removeClass('f-hide');
                                        if(!selected) {
                                            selected = true;
                                            itemBlock.addClass('hl-item-selected');
                                        } else {
                                            itemBlock.removeClass('hl-item-selected');
                                        }
                                    } else {
                                        itemBlock.addClass('f-hide');
                                        itemBlock.removeClass('hl-item-selected');
                                    }
                                });
                            } else {
...
/**
 * Created by hero on 2014/9/1 0001.
 * 处理滚动加载相关事宜
 */
define('widget/scroller',function(require, exports, module) {
    var tool = require('tool/tool');
    return function(jquery){
        (function($) {
            var SCROLLER_KEY = 'data-scroller-key';
            var SCROLL_PREFER = 1000;
            var offsetCache = {};
            function getOffset(dom, key) {
                if(!offsetCache[key]) {
                    offsetCache[key] = dom.offset();
                }
            ...
            $MQ.LAZYLOAD.bind(function(evt, pos) {
                clearCache();
            });
            $.fn.extend({
                /**
                 * 绑定滚动
                 * @param scrollIn {Function} 滚动进入
                 * @param scrollOut {Function} 滚动出去
                 */
                bindScroller : function(scrollIn, scrollOut) {
                    var dom = this;
                    // 已经绑定过
                    if(dom.attr(SCROLLER_KEY)) {
                        return;
                    }

                    var height = dom.height();
                    var key = tool.uid();
                    dom.attr(SCROLLER_KEY, key);

                    $MQ.SCROLL.bind(function(evt, pos) {
                        var offset = getOffset(dom, key);
                        var bodyTop = offset.top;
                        var bodyBottom = bodyTop + height;
                        if(bodyBottom < pos.top - SCROLL_PREFER || bodyTop > pos.bottom + SCROLL_PREFER) {
                            // 在外边了
                            scrollOut(dom);
                        } else {
                            scrollIn(dom);
                 ...
                 * 解绑滚动
                 */
                unbindScroller : function() {
                    var dom = this;
                    // 已经绑定过
                    var key = dom.attr(SCROLLER_KEY);
                    if(key) {
                        $MQ.SCROLL.unbind(key);
                        // 移除attr
                        dom.removeAttr(SCROLLER_KEY);
                    }
                }
...
define('widget/lazyload',function(require, exports, module) {
    return function(jquery){
        (function($) {
            function bindImgs(dom) {
                var imgList = [];

                var imgs = $('img.js-lazyload', dom);
                if(imgs.length > 0) {
            ...
            function bindImg(img) {
                img.bindScroller(function(dom) {
                    dom.unbindScroller();
                    var url = dom.attr('data-src');
                    dom.removeAttr('data-src');
                    dom.removeClass('js-lazyload');
...
define('widget/interval',function(require, exports, module) {
    return function(jquery){
        (function($) {
            var AUTOSHOWCASEKEY = 'autoShowcases';
            var AUTOSHOWCASETIMEOUT = 'autoShowcasesTimeout';
            var INTERVAL = 5000;
            function showCase (container, template, datalist, blockSize, index) {
                // 如果不可滚动继续  setTimeout
                if(!container.data(AUTOSHOWCASEKEY)) {
                    var timeout = setTimeout(function() {
                        showCase(container, template, datalist, blockSize, index);
                    }, INTERVAL);
                    container.data(AUTOSHOWCASETIMEOUT, timeout);
                    // console.log('showcase sleep ' + index);
                    return;
                }
                // console.log('showcase ' + index);
                var data = datalist.slice(index * blockSize, (index + 1) * blockSize)
                var html = $.jqote(template, {result : data});
                var dom = $(html);
                ...
                var block = Math.floor(datalist.length / blockSize);
                if(block <= 1) {
                    return;
                }
                index ++;
            ...
            $.fn.extend({
                startInterval: function(template, datalist, blockSize) {
                    var container = this;
                    container.stopInterval();
                    
                    container.hover(function() {
                        container.data(AUTOSHOWCASEKEY, false);
                    }, function() {
                        container.data(AUTOSHOWCASEKEY, true);
                    });

                    container.bindScroller(function(block) {
                        container.data(AUTOSHOWCASEKEY, true);
                    }, function() {
                        // 停止滚动
                        container.data(AUTOSHOWCASEKEY, false);
                    ...
                    // 启动自动轮播
                    container.data(AUTOSHOWCASEKEY, true);
                    showCase(container, template, datalist, blockSize, 0);
                },
                stopInterval : function() {
                    var container = this;
                    container.unbind('hover');
                    container.unbindScroller();
                    container.data(AUTOSHOWCASEKEY, false);
                    var timeout = container.data(AUTOSHOWCASETIMEOUT);
...
define('widget/slide', function(require, exports, module) {
    return function(jquery) {
        (function($) {
            var SUPPORT_AUTO = 'data-autoslide'
            var AUTOSHOWCASEKEY = 'autoSlide';
            var AUTOSHOWCASETIMEOUT = 'autoSlideTimeout';
            var INTERVAL = 5000;
            var ANIMATETIME = 800;

            function showCase(container, index, manual, isRtl) {
                var supportAuto = container.attr(SUPPORT_AUTO) == "true";
                if(supportAuto) {
                    var timeout = container.data(AUTOSHOWCASETIMEOUT);
                    clearTimeout(timeout);
                    // 自动的
                    if (!manual && !container.data(AUTOSHOWCASEKEY)) {
                        var timeout = setTimeout(function() {
                            // wait
                            showCase(container, index, false, true);
                        }, INTERVAL);
                        container.data(AUTOSHOWCASETIMEOUT, timeout);
                        return;
                    }
                ...
                // 如果不可滚动继续  setTimeout
                var slidewrap = container.find('.js-slidewrap');
                var slidectrl = container.find('.js-slidectrl-num');
                var slides = slidewrap.children('.slide');
                var currentIndex = parseInt(slidectrl.find('.current').attr('data-index'), 10);
                var nextIndex = index;
                ...
                var currentBlock = $(slides[currentIndex]);
                var nextBlock = $(slides[nextIndex]);

                var left = 0;
                // var isRtl = true;
                // if(manual && nextIndex < currentIndex) {
                //     isRtl = false;
                // }
                // 自动的从右往左
                if (isRtl) {
                ...
                slidectrl.find('.current').removeClass('current');

                slidectrl.find('[data-index="' + nextIndex + '"]').addClass('current');
                nextBlock.animate({
                    left: 0
                }, ANIMATETIME, function() {
                });
                ...
                if(supportAuto) {
                    var timeout = setTimeout(function() {
                        showCase(container, nextIndex + 1, false, true);
                    }, INTERVAL);
                    container.data(AUTOSHOWCASETIMEOUT, timeout);
                }
            }
            ...
            $.fn.extend({
                ...
                startSlide: function() {
                    ...
                    if(supportAuto) {
                        setTimeout(function() {
                            showCase(container, 1, false, true);
                        }, INTERVAL);
                    }
                    // 启动自动轮播
                    container.data(AUTOSHOWCASEKEY, true);

                    var slidectrlnum = container.find('.js-slidectrl-num');
                    var slidectrlbtn = container.find('.js-slidectrl-btn');
                    slidectrlnum.delegate('.js-ctrl', 'click', function(evt) {
                        var btn = $(evt.currentTarget);
                        if(!btn.hasClass('current')) {
                            var nextIndex = parseInt(btn.attr('data-index'), 10);
                            var currentIndex = parseInt(slidectrlnum.find('.current').attr('data-index'), 10);
                            var isRtl = nextIndex > currentIndex;
                            showCase(container, nextIndex, true, isRtl);
                        }
                    });
                    // 左右按钮
                    slidectrlbtn.delegate('.js-ctrl', 'click', function(evt) {
                        var btn = $(evt.currentTarget);
                        var action = btn.attr('data-action');
                        var currentIndex = parseInt(slidectrlnum.find('.current').attr('data-index'), 10);
                        switch (action) {
                            case 'next':
                                showCase(container, currentIndex + 1, true, true);
                                break;
                            case 'prev':
                                showCase(container, currentIndex - 1, true, false);
                                break;
                        }
                    });
                },
                slide2Page : function(nextIndex) {
                    var container = this;
                    showCase(container, nextIndex, true, true);
                }
            });
        })(jquery);
    };
...
define('widget/fullpage', function(require, exports, module) {
    return function(jquery) {
        (function($) {
            // 是否在滚动
            var isPageScrolling = false;

            function showCase(container, currentPage, nextPage) {
                var winHeight = $(window).height();
                isPageScrolling = true;
                container.animate({top : - nextPage * winHeight}, 600, function() {
                    var sections = container.children('.js-page-section');
                    $(sections[currentPage]).removeClass('current');
                    $(sections[nextPage]).addClass('current');
                    // 按钮的变化
                    var pagectrl = container.siblings('.g-fullpage-ctrl');
                    var numctrl = pagectrl.children('.js-ctrl-num');
                    var ctrls = numctrl.children('.js-ctrl');
                    $(ctrls[currentPage]).removeClass('current');
                    $(ctrls[nextPage]).addClass('current');
                    isPageScrolling = false;
                });

            }

            function scroll(container, delta) {
                var currentPage = parseInt(container.children('.js-page-section.current').attr('data-index'), 10);
                var nextPage = -1;
                // 3,6,9
                if(delta < 0) {
                    var sections = container.children('.js-page-section');
                ...
                if(nextPage >= 0) {
                    showCase(container, currentPage, nextPage);
                }
            }
            $.fn.extend({
                startFullPage: function() {
                    ...
                    $MQ.RESIZE.bind(function(evt, data) {
                        var currentPage = parseInt(container.children('.js-page-section.current').attr('data-index'), 10);
                        container.css({top : - currentPage * data.height});
                    });
                    $(document.body).bind("mousewheel DOMMouseScroll", function(event) {
                        if(!isPageScrolling) {
                            ...
                            var delta = nativeEvent.wheelDelta ? nativeEvent.wheelDelta : - nativeEvent.detail;
                    ...
                    var pagectrl = container.siblings('.g-fullpage-ctrl');
                    var numctrl = pagectrl.children('.js-ctrl-num');
                    numctrl.delegate('.js-ctrl', 'click', function(evt) {
                        if(!isPageScrolling) {
                            var btn = $(evt.currentTarget);
                            if(!btn.hasClass('current')) {
                                var nextIndex = parseInt(btn.attr('data-index'), 10);
                                var currentIndex = parseInt(numctrl.find('.current').attr('data-index'), 10);
                                showCase(container, currentIndex, nextIndex);
                            }
                        }
                    }) ;
                    ...
                    if(isTouchDevice && isTouch) {
                        ...
                        $(document.body).bind('touchend', function(evt) {
                            if(evt.originalEvent.touches.length > 0) {
                                startY = 0;
                                startX = 0;
                            }
...
define('widget/input',function(require, exports, module) {
    var Validate = require('tool/validate');
    return function(jquery){
         (function($) {
            function showError(ele, msg) {
            ...
            function togglePlacholder(input, placeholder) {
                if(input.val() == '') {
                    placeholder.show();
                } else {
                    placeholder.hide();
            ...
            var PLUGIN_INPUT = 'PLUGIN_INPUT';
            $.fn.extend({
                startInput : function() {
                    var wrap = $(this);
                    if(wrap.data(PLUGIN_INPUT)) {
                        return;
                    }
                    wrap.data(PLUGIN_INPUT, true);

                    var ipt = wrap.find('input');
                    var placeholder = wrap.find('.placeholder');
                    placeholder.click(function() {
                        ipt.focus();
                    });
                    ipt.focus(function() {
                        togglePlacholder(ipt, placeholder);
                    });
                    ...
                    ipt.bind('propertychange, input, keyup', function() {
                        togglePlacholder(ipt, placeholder);
                        ipt.hideError();
                    });
                    // ipt.showError('test')
 ...
 * @public
 */
define('ui/registercache', function(require, exports, module) {
    var urlUtil = require('tool/url');
    var cache = {
        init: function() {
            // body...
        },
        getHost : function() {
            // if(location.host === '18.mail.163.com') {
            return 'https://ssl.mail.163.com/security.mail.163.com';
            // }
            // return 'http://security.mail.163.com:8080';
        },
        /**
         * 下发验证码
         *
         ...
         * 
         * @param  {Object} param               
         * @param  {string}   param.mobile      必填，手机号码
         *
         * @return {Deferred}               回调处理
         */
        getCode: function(param) {
            var that = this;
            var _param = param || {};
            var defer = $.Deferred();
            var _url = this.getHost() + '/mobileserv/appCodeSend.do';
            _url = urlUtil.appendURL(_url, {
                mobile : _param.mobile,
                product : 'qingning'
            });
            var _settings = {
                callbackName : 'cb'
            ...
            var _callback = function(res) {
                if(res.code === "0") {
                    ...
                    // -1\nwrong number     手机号码参数不正确
                    // -1\nparams error     产品号下发禁止
                    // -2\ntimes over   当天（24小时内）下发验证码次数达到上限
                    // -2\ntoo frequent     下发验证码过于频繁（距离上一次下发少于60s）
                    if(res.code === '-1') {
                        defer.reject(res.code, '手机号码不正确');
                    } else if(res.code === '-2') {
                        defer.reject(res.code, '下发验证码过于频繁');
                    } else {
                        defer.reject(res.code, '验证码发送不成功');
                    }
                }
            }
            urlUtil.jsonp(_url, _callback, _settings);
         ...
         * @param  {void} param     暂无
         * @param  {string}   param.mobile      必填，手机号码
         * @param  {string}   param.vcode      必填，验证码
         * 
         * @return {Deferred}       回调处理
         */
        validate: function(param) {
            ...
            var _param = param || {};
            var defer = $.Deferred();
            var _url = this.getHost() + '/mobileserv/msCodeValidate.do';
            _url = urlUtil.appendURL(_url, {
                mobile : _param.mobile,
                vcode : _param.vcode,
                product : 'qingning'
            ...
            var _callback = function(res) {
                if(res.code === "0") {
                    ...
                    if(res.code === '-1') {
                        defer.reject(res.code, '验证码错误');
                    } else if(res.code === '-3') {
                        var forgetUrl = 'http://reg.163.com/getpasswd/RetakePassword.jsp?from=youpin';
                        defer.reject(res.code, '手机号码已被开通手机邮，请直接<a href="javascript:;" hidefocus="true" id="backLoginBtn">登录</a>或者<a href="' + forgetUrl + '" target="_blank" hidefocus="true">找回密码</a>');
                    } else if(res.code === '-6') {
                        defer.reject(res.code, '验证码错误次数过多');
                    } else {
                        defer.reject(res.code, '请重新获取验证码');
                    }
                }
            }
            urlUtil.jsonp(_url, _callback, _settings);
         ...
         * @param  {void} param     暂无
         * @param  {string}   param.mobile      必填，手机号码
         * @param  {string}   param.md5sign      接口msCodeValidate返回的对应值
         * @param  {string}   param.validfrom     接口msCodeValidate返回的对应值
         * @param  {string}   param.pawd      用户设置的手机号码邮箱密码
         * 
         * @return {Deferred}       回调处理
         */
        register: function(param) {
            ...
            var _param = param || {};
            var defer = $.Deferred();
            var _url = this.getHost() + '/mobileserv/msActivateMm.do';
            _url = urlUtil.appendURL(_url, {
                mobile : _param.mobile,
                md5sign : _param.md5sign,
                validfrom : _param.validfrom,
                pawd : _param.pawd,
                product : 'qingning'
            });
            var _settings = {
            ...
            var _callback = function(res) {
                if(res.code === "1") {
                    ...
                    if(res.code === '3') {
                        defer.reject(res.code, '手机号码邮箱已被占用');
                    } else if(res.code === '-1') {
                        defer.reject(res.code, '参数有误');
                    } else if(res.code === '-2') {
                        defer.reject(res.code, '密码少于6位');
                    } else if(res.code === '-6') {
                        defer.reject(res.code, '激活的错误次数过多');
                    } else if(res.code === '-4') {
                        defer.reject(res.code, 'token无效');
                    } else {
                        defer.reject(res.code, '注册失败');
                    }
                }
            }
            urlUtil.jsonp(_url, _callback, _settings);
 ...
 * @public
 */
define('ui/logindialog', function(require, exports, module) {
    var registerCache = require('ui/registercache');
    var validate = require('tool/validate');
    var Tool = require('tool/tool');
    var logind = {
        init: function() {
            ...
            $MQ.REQUIRELOGIN.bind(function(evt, data) {
         ...
         * 打开弹窗
         */
        showDialog: function(isLogin) {
            var that = this;
            var loginDialog = $($.jqote($('#loginTemplate').html(), {}));

            loginDialog.showDialog({
                clsName: 'm-popwin-login'
            });

            var loginForm = loginDialog.find('form[name="loginForm"]');
            var registerForm = loginDialog.find('form[name="registerForm"]');

            if (!isLogin) {
                loginForm.addClass('f-hide');
                registerForm.removeClass('f-hide');
            }
            // 切换到注册
            loginDialog.delegate('#registerBtn', 'click', function() {
                loginForm.addClass('f-hide');
                registerForm.removeClass('f-hide');
            });
            // 切换到登录
            loginDialog.delegate('#backLoginBtn', 'click', function() {
                registerForm.addClass('f-hide');
                loginForm.removeClass('f-hide');
            });

            // 初始化内容
            var aPinfo = (Tool.getCookie('P_INFO') || '').split('|');
            var username = '';
            var usernameIpt = loginForm.find('[name="username"]');
            var pwdIpt = loginForm.find('[name="password"]');
            if (aPinfo[0]) {
                username = aPinfo[0];
                if (username) {
            ...
            // 密码处回车
            loginForm.find('[name="password"]').on('keypress', function(e) {
                e = e || window.event;
                if ((e.which || e.keyCode) == 13) {
                    loginForm.submit();
                }
            });
            loginForm.delegate('#loginBtn', 'click', function() {
                loginForm.submit();
                return false;
            });

            loginForm.delegate('#savelogin', 'click', function(evt) {
                var savelogin = $(evt.currentTarget);
            ...
            // 回车，聚焦到密码
            usernameIpt.bind('keyup', function(evt) {
                if (evt.keyCode == 13) {
                    if (usernameIpt.val().indexOf('@') > 0) {
                        pwdIpt.focus();
                    }
                }
            });
            usernameIpt.parent().parent().regSuggest(loginForm.find('#usernameAutocomplete'));
            // 登录
            loginForm.submit(function() {
                var data = loginForm.formValidate();
                if (data) {
                    var username = data.username;

                    if (username.indexOf("@") == -1) {
                        if (!validate.isCellPhone(username)) {
                            usernameIpt.showError("请填写正确的手机号码");
                            return false;
                        } else {
                            return true;
                        }
            ...
            // 绑定注册相关
            var mobileIpt = registerForm.find('[name="mobile"]');
            var passwordIpt = registerForm.find('[name="password"]');
            var passwordIpt2 = registerForm.find('[name="password2"]');
            var vcodeIpt = registerForm.find('[name="vcode"]');
            var vcodeBtn = registerForm.find('#vcodeBtn');
            var registerBtn = registerForm.find('#registerBtn');
            // 绑定事件
            registerForm.formValidate();

            vcodeBtn.click(function() {
                vcodeIpt.hideError();
                that.getVcode(mobileIpt, vcodeBtn);
            });
            mobileIpt.blur(function() {
                var mobile = mobileIpt.val();
                if ($.trim(mobile) === '') {
                    mobileIpt.showError("请输入手机号码");
                } else if (!validate.isCellPhone(mobile)) {
                    mobileIpt.showError("请填写正确的手机号码");
                }
            });
            // 检查密码.
            passwordIpt.blur(function() {
                var pw1 = passwordIpt.val();
                if (pw1.length < 6 || pw1.length > 16) {
                    passwordIpt.showError('密码长度应为6~16个字符');
                }
                var pw2 = passwordIpt2.val();
                if (pw2 !== '' && pw1 !== pw2) {
                    passwordIpt2.showError('两次填写的密码不一致');
                }
            });
            passwordIpt.focus(function() {
                passwordIpt.hideError();
            });
            passwordIpt2.blur(function() {
                var pw2 = passwordIpt2.val();
                var pw1 = passwordIpt.val();
                if (pw1 !== pw2) {
                    passwordIpt2.showError('两次填写的密码不一致');
            ...
            passwordIpt2.focus(function() {
                passwordIpt2.hideError();
            });
            ...
            registerBtn.click(function() {
                that.login({
                    vcodeIpt: vcodeIpt,
                    passwordIpt2: passwordIpt2,
                    passwordIpt: passwordIpt,
                    mobileIpt: mobileIpt,
                    usernameIpt: usernameIpt,
                    pwdIpt: pwdIpt,
                    loginForm: loginForm
                });
            });
        ...
        /**
         * 获取
         * @param  {[type]} mobileIpt [description]
         * @param  {[type]} vcodeBtn  [description]
         * @return {[type]}           [description]
         */
        getVcode: function(mobileIpt, vcodeBtn) {
            var that = this,
                vcodeSpan = vcodeBtn.find('span'),
                mobile = mobileIpt.val();

            if (this.isStarted) {
                return;
            }
            if (that.vcodeInterval) {
                window.clearInterval(that.vcodeInterval);
                vcodeSpan.text('获取验证码');
            }
            that.isStarted = true;
            if (validate.isCellPhone(mobile)) {
                that.count = 59;
                that.vcodeInterval = window.setInterval(function() {
                    if (that.count <= 0) {
                        vcodeSpan.text('获取验证码');
                    ...
                    vcodeSpan.text(that.count + '秒后再次获取');
                    that.count--;
                }, 1010);
                registerCache.getCode({
                    mobile: mobile
                }).done(function(res) {
                    that.isStarted = false;
        ...
        login: function(inputs) {
            var that = this,
                confirm = true,
                vcodeIpt = inputs.vcodeIpt,
                passwordIpt = inputs.passwordIpt,
                passwordIpt2 = inputs.passwordIpt2,
                mobileIpt = inputs.mobileIpt,
                usernameIpt = inputs.usernameIpt,
                pwdIpt = inputs.pwdIpt,
                loginForm = inputs.loginForm;
            var pw1 = passwordIpt.val();
            if (pw1.length < 6 || pw1.length > 16) {
                passwordIpt.showError('密码长度应为6~16个字符');
                confirm = false;
            ...
            var vcode = $.trim(vcodeIpt.val());
            if (vcode === '') {
                vcodeIpt.showError('请输入验证码');
                confirm = false;
            }
            var mobile = mobileIpt.val();
            if ($.trim(mobile) === '') {
            ...
            if (confirm) {
                // 先验证code
                registerCache.validate({
                    mobile: mobile,
                    vcode: vcode
                }).done(function(res) {
                    // b079e92203f4d06648084079f75f66de#1405067151570
                    if (typeof res.desc !== 'string') {
                        // error
                    }
                    var desc = res.desc.split('#');
                    if (desc.length !== 2) {
                        // error
                    }
                    var md5sign = desc[0];
                    var validfrom = desc[1];
                    registerCache.register({
                        ...
                        usernameIpt.val(mobile + '@163.com');
                        pwdIpt.val(pw1);
                        // 处理一下地址,加上from=login，区分来源
                        // loginForm.find('[name="url"]').val('http://' + location.host + '/entry.do?from=login');
                        loginForm.submit();
                    }).fail(function(code, errorText) {
                        if (code === '3') {
                            mobileIpt.showError(errorText);
                        } else {
                            vcodeIpt.showError(errorText);
                        }
                    });
...
define('main', function(require, exports, module) {
    ...
    var main = {
        ...
        init: function() {
            ...
            require('widget/form')($);
            require('widget/select')($);
            require('widget/countdown')($);
            require('widget/dropmenu')($);
            require('widget/hovermenu')($);
            require('widget/socialshare')($);
            require('widget/pager')($);
            require('widget/regsuggest')($);
            require('widget/scroller')($);
            require('widget/lazyload')($);
            require('widget/interval')($);
            require('widget/slide')($);
            require('widget/fullpage')($);
            require('widget/input')($);

            require('ui/logindialog').init();
            ...
            win.resize(function() {
                ...
                loadTimeout = setTimeout(function() {
                    var pos = getPos(win);
                    $MQ.SCROLL.notify(pos);
                    $MQ.RESIZE.notify(pos);
                }, 30);
            });
            win.scroll(function() {
                clearTimeout(loadTimeout);
                loadTimeout = setTimeout(function() {
                    $MQ.SCROLL.notify(getPos(win));
                }, 30);
            });
            // 对图片进行lazyload
            $(document.body).lazyload();
            $MQ.LAZYLOAD.bind(function(evt, pos) {
                // 重新绑定一下
                $(document.body).lazyload();
                $MQ.SCROLL.notify(getPos(win));
            ...
            $('[data-plugin="slide"]').each(function(i, d) {
                $(d).startSlide();
            });

            $('[data-plugin="input"]').each(function(i, d) {
                $(d).startInput();
            });
            $('[data-plugin="dropmenu"]').each(function(i, d) {
                var m = $(d);
                var refer = $(m.attr('data-plugin-refer'));
                m.dropMenu(refer);
            });
            $('[data-plugin="hovermenu"]').each(function(i, d) {
                var m = $(d);
                var refer = $(m.attr('data-plugin-refer'));
                m.hoverMenu(refer);
            });
            $('[data-plugin="dialog"]').each(function(i, d) {
                var m = $(d);
                m.click(function() {
                    var refer = $($(m.attr('data-plugin-refer')).html());
                    refer.showDialog();
                    // 初始化相关的dialog
                });
            });
            // 绑定
            $(document.body).delegate('[data-log="true"]', 'click', function(evt) {
                var btn = $(evt.currentTarget);
                var logName = btn.attr('data-logName');
                if (logName) {
                    Log.log({
                        key: logName
                    });
