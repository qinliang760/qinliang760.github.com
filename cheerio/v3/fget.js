...
//
//var dir='';
var count=[],finishCount=[];
var download = function(url, dir_p, filename,cb){
    var path=dir_p;
    mkdirp(path);
    ...
    //request.head(url, function(err, res, body){
      ...
      var sendReq = request.get(url);
          ...
          file.on('finish', function() {
            finishCount.push(filename);
            console.log(finishCount.length+"/"+count.length);
            if(finishCount.length==count.length){
              console.log(process.argv[2]+"全部下载完成");
            }
...
//app.get('/', function (req, res, next) {
  ...
  superagent.get('https://playoverwatch.com/zh-tw/heroes/'+process.argv[2]+'/')
    .end(function (err, sres) {
      ...
      $("[data-story-bg]").each(function(k,v){
          var src=$(v).attr("data-story-bg");
          var lt=src.lastIndexOf("/");
          var dir_p=getImg(src,lt);
          if(dir_p){

            download(src,dir_p,src.substring(lt));
            var pc_pic=src.substring(lt).replace(/-mobile/,"");
            var pc_pic_url=src.replace(/-mobile/,"");
            download(pc_pic_url,dir_p,pc_pic);
            //$(v).attr("src","http://overwatch.nos.netease.com/1/assets/"+dir_p+src.substring(lt));
          }
      ...
      fs.writeFile(path+"/"+process.argv[2]+".shtml",html,{ encoding: 'utf8'},function(err){
          if(!err)
          console.log("写入成功！")
      })
    });
